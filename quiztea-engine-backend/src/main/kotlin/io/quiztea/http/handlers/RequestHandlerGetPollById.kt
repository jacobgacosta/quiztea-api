...
class RequestHandlerGetPollById : RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    override fun handleRequest(input: APIGatewayProxyRequestEvent?, context: Context?): APIGatewayProxyResponseEvent {
        return try {
            ...
            APIGatewayProxyResponseEvent().apply {
                statusCode = if (result.isEmpty()) 404 else 200
                headers = input.headers?.toMap() ?: emptyMap()
                body = result.ifEmpty { null }
            }
